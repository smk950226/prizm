                    ...
                    <div>
                        {chatList && chatList.length > 0 ? (
                            chatList.map((chat, index) => (
                                <div key={index} className={`${index === chatList.length - 1 ? null : styles.borderBtmGrayDc} ${styles.py3} ${styles.row} ${styles.mx0} ${styles.alignItemsCenter} ${styles.justifyContentBetween} ${styles.cursorPointer}`} onClick={chat.order.status === 'confirmed' ? () => this.props.goMessageDetail(chat.id, chat.order) : null}>
