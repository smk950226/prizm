    ...
    render(){
        ...
        return(
            <div className={`${styles.safearea} ${styles.containerCustomer}`}>
                <p className={`${styles.fontBold} ${styles.font20} ${styles.px3}`} style={(isLoggedIn && notification && showNotification) ? {marginTop: 30} : {marginTop: 80}}>{isLoggedIn ? profile ? profile.name : this.context.t("Sign In to view your profile") : this.context.t("Sign In to view your profile")}</p>
                {isLoggedIn ? (
                    <Fragment>
