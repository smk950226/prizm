        ...
        {props.loading ? (
            ...
            <Fragment>
                ...
                {props.order.status === 'confirmed' && (
                    <div className={`${styles.widthFull} ${styles.bgConfirmed} ${styles.row} ${styles.mx0} ${styles.alignItemsCenter} ${styles.justifyContentCenter} ${styles.btn} ${styles.mt3}`} onClick={props.isDeposit ? () => props.payDeposit(props.order.option.price +  Math.ceil(props.order.option.price*0.1)) : () => props.payPaypal(props.order.option.price +  Math.ceil(props.order.option.price*0.1))} style={{height: 48}}>
